...
declare @x varchar(50)= '1234ABC123456XYZ1234567890ADS'
set @x = translate('1234ABC123456XYZ1234567890ADS','0123456789', 'XXXXXXXXXX')
select @x
...
select 
		texts, len(texts)-len(replace(texts,' ',''))
from CountSpaces
...
select EMPLOYEE_ID, FIRST_NAME, LAST_NAME, HIRE_DATE, datediff(year,HIRE_DATE,getdate()) as year_of_service  from Employees
where datediff(year,HIRE_DATE,getdate()) between 10 and 15

--------------------------------------------------------------------------

declare @a varchar(50) = 'rtcfvty34redt'

select
translate(@a, '0123456789', replicate(' ',10)) as letters,
translate(@a, 'abcdefghijklmnopqrstuvwxyz', replicate(' ',26)) as numbers

--------------------------------------------------------------------------

select * from weather w2
join weather w1 on w1.RecordDate=dateadd(day,1,w2.RecordDate)
...
select value from fruits
cross apply string_split(fruit_list,',',1) as s
where s.ordinal=3

--------------------------------------------------------------------------

declare @w varchar(50) = 'sdgfhsdgfhs@121313131'

select 

s.ordinal,
s.value as character 
from string_split(@w,'', 1) as s
order by s.ordinal

--------------------------------------------------------------------------

select p1.id,case
		when p1.code=0 then p2.code else p1.code end
...
select FIRST_NAME, case 
			...
			when datediff(year,HIRE_DATE,getdate()) > 20 then 'Veteran' end as stage
...
try_cast(left(vals,
				case
					when patindex('%[0-9]%', vals + 'x')>1
					then patindex('%[0-9]%', vals + 'x')-1
				else 0
				end 
				)as 
		int) as StartingInt
...
WITH day_area AS (
  ...
  SELECT
    ...
    date,
    area,
    SUM(COALESCE(sales_amount,0)) AS area_sales_day
  FROM WeekPercentagePuzzle
  GROUP BY financialweek, date, area
)
SELECT
  da.date,
  da.financialweek,
  ...
  da.area_sales_day * 100.0
    / NULLIF(SUM(da.area_sales_day) OVER (PARTITION BY da.financial_week), 0) AS pct_of_week
FROM day_area da
ORDER BY da.financialweek, da.date, da.area;
