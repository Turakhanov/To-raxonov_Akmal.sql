...
select p.ProductName, s.SupplierName from products as p
 ...
 select coalesce(c.FirstName,c.LastName) as Names, o.OrderID as OrderID  from orders as o
 ...
 select s.Name, enr.CourseID from Students as s
 join Enrollments as enr on enr.StudentID=s.StudentID

 --------------------------------------------------------------------

 select o.OrderID from Payments as p
...
select coalesce(c.FirstName,c.LastName), s.ProductID from Customers as c
join sales as s on s.CustomerID=c.CustomerID
where s.SaleAmount>=500

----------------------------------------------------------------------

select s.Name, s.Major from Students as s
join Enrollments as enr on enr.StudentID=s.StudentID
...
select Departmentname, e.Name from Departments as d
join Employees as e on e.DepartmentID=d.DepartmentID
...
select coalesce(c.FirstName,c.LastName) as Name, s.SaleAmount, c.Country from sales as s
join Customers as c on c.CustomerID=s.CustomerID
where c.Country = 'USA'

----------------------------------------------------------------------

select o.OrderID, c.Country, o.TotalAmount from Customers as c
join orders as o on o.CustomerID=c.CustomerID
where o.TotalAmount>100

----------------------------------------------------------------------

select e1.Name, e1.DepartmentID, e2.Name, e2.DepartmentID from Employees as e1
join Employees as e2 on e2.DepartmentID<>e1.DepartmentID

----------------------------------------------------------------------

select * from Payments as py
...
select e1.Name as manager,e1.Salary, e2.Name, e2.Salary from Employees as e1
left join Employees as e2 on e2.ManagerID=e1.EmployeeID
where e1.Salary<=e2.Salary

----------------------------------------------------------------------

select coalesce(c.FirstName,c.LastName) from orders as o
