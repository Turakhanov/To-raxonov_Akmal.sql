...
select count(ProductID) as Total_Products, category from products
group by category
...
select coalesce(FirstName,lastname), city from customers
where city like '%a'
...
select count(CustomerID) as Total_Customer, country from customers
group by Country
...
select o.CustomerID, o.OrderDate, i.InvoiceDate from Orders as O
join Invoices as I on I.CustomerID=o.CustomerID
where o.OrderDate between '01/01/2023' and '02/01/2023'
...
select avg(OrderID) as avg, year(OrderDate) from orders
...
select count(saleid) as total_sales, ProductID from sales
group by ProductID
...
select * from(
select district_name, population, year from city_population) as src
pivot( max(population) for district_name in ([Chilonzor], [Yakkasaroy], [Mirobod], [Yashnobod],[Bektemir]) )as Population_Each_City
...
unpivot(
	population for year in ([2012],[2013])) as city_population
