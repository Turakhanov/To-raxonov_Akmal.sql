...
select top 1 coalesce(FirstName, LastName) from employees
...
-----------------------------------------------------
select count(EmployeeID) as NumberofEmployees, DepartmentName from employees
group by DepartmentName
-----------------------------------------------------
select max(price) as MAXprice, min(price) MINprice,category from products
group by category
-----------------------------------------------------
...
group by DepartmentName
-----------------------------------------------------
select DepartmentName, Avg(salary) as AVGsalary, count(EmployeeID) NumberofEmployees from employees
group by DepartmentName
-----------------------------------------------------
...
-----------------------------------------------------
select sum(saleamount) as '2023' from sales
...
having count(*) >= 3
-----------------------------------------------------
select Departmentname, avg(salary) as SalaryExpence from employees
group by departmentname
...
-----------------------------------------------------
select CustomerID, (TotalAmount/Quantity) as BigPurchase, min(TotalAmount/Quantity) as LeastPurchase from orders
group by customerid, (TotalAmount/Quantity)
having (TotalAmount/Quantity)>50
-----------------------------------------------------
select 
