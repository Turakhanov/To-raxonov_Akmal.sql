...
INSERT INTO Employees_demo (EmployeeID, FirstName, LastName, DepartmentName, Salary, HireDate, Age, Email, Country) VALUES
...
select top 5 * from Employees_demo
...
INSERT INTO Customers_info (CustomerID, FirstName, LastName, Email, Phone, Address, City, State, PostalCode, Country) VALUES
...
INSERT INTO Products_github VALUES
...
CREATE TABLE Orders_info (
    ...
    FOREIGN KEY (ProductID) REFERENCES Products_github(ProductID)
);
...
select Email from Employees_demo
...
select distinct ProductName,Category from Products_Discounted
order by ProductName desc

select top 10 * from Products_Discounted 
order by price desc

select coalesce(FirstName,LastName) from Employees_demo
...
select * from Employees_demo
select coalesce(Firstname,' ')+' '+coalesce(LastName,' ') as FullName from Employees_demo 
...
select * from Employees_demo
where Salary > all(select salary from Employees_demo where DepartmentName = 'Sales')

SELECT * FROM Sales
WHERE SaleDate BETWEEN DATEADD(DAY, -180, (SELECT MAX(SaleDate) FROM Sales))
                 AND (SELECT MAX(SaleDate) FROM Sales)
